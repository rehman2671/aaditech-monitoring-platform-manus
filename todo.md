...
- [ ] Implement bounded historical process performance telemetry with truthful unavailable fields and configurable retention; raw persistence, retention cleanup, tenant-scoped 5-minute/15-minute/hour/day/week bucket queries, and endpoint-detail 5-minute trend rendering are complete, while longer-term rollup policy remains.
- [ ] Implement confidence-aware application grouping plus separate application and process views.
- [ ] Implement deterministic CPU/RAM/storage/battery/hardware intelligence and threshold-based findings without LLM-owned metrics; CPU/RAM/disk threshold observations and explicit missing-metric findings are implemented, the live endpoint now shows real RAM/CPU/network evidence, and the battery UI renders absent charge as Unavailable, while actual battery collection and hardware threshold findings remain.
...
- [ ] Add analyst frontend views, finding details, evidence/provenance, confidence, recommendations, and explicit no-action states; endpoint detail now includes an evidence-bound AI Analyst panel with on-demand/cached actions, confidence, findings, limitations, and no-action rendering, while fleet-level views remain.
