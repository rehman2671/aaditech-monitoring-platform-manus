      ...
      {canWrite && (
        <div className="bg-slate-900/70 border border-slate-800 rounded-2xl p-6 shadow-2xl space-y-5">
          ...
          <div className="border-t border-slate-800 pt-4">
            ...
            {builds.length === 0 ? <p className="text-xs text-slate-500">No MSI builds have been queued for this organization.</p> : <div className="space-y-2">{builds.slice(0, 5).map(job => <div key={job.id} className="flex flex-col md:flex-row md:items-center justify-between gap-3 rounded-xl border border-slate-800 bg-slate-950/60 p-3 text-xs"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="font-mono text-slate-200">v{job.agentVersion}</span><span className={`rounded-full px-2 py-0.5 border ${job.status === 'succeeded' ? 'text-emerald-400 border-emerald-500/20 bg-emerald-500/10' : job.status === 'failed' ? 'text-rose-400 border-rose-500/20 bg-rose-500/10' : 'text-amber-400 border-amber-500/20 bg-amber-500/10'}`}>{job.status}</span><span className="text-slate-500">{job.signMode}</span>{job.isSigned && <BadgeCheck className="w-3.5 h-3.5 text-emerald-400" />}</div><p className="text-slate-500 mt-1 truncate">{job.artifactFilename ?? job.errorMessage ?? `Queued ${formatDate(job.createdAt)}`}</p></div><div className="flex items-center gap-3 shrink-0"><span className="text-slate-500 font-mono">{formatBytes(job.sizeBytes)}</span>{job.status === 'succeeded' ? <Button size="sm" onClick={() => void handleDownload(job)} disabled={downloadingJobId === job.id} className="h-8 bg-emerald-600 hover:bg-emerald-500 text-white gap-1.5">{downloadingJobId === job.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Download className="w-3.5 h-3.5" />} Download MSI</Button> : <span className="inline-flex items-center gap-1.5 rounded-md border border-slate-700 px-2.5 py-1.5 text-[11px] text-slate-500"><Download className="w-3.5 h-3.5" /> Download after success</span>}</div></div>)}</div>}
          </div>
        </div>
      )}
